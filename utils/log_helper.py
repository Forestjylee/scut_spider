...
def get_logger(
    ...
    if to_console:
        logger.addHandler(
            _get_log_handler(handler_object=logging.StreamHandler, level=level)
        )
    if to_file:
        logger.addHandler(
            _get_log_handler(
...
def _get_log_handler(handler_object, file_name=None, level=logging.INFO):
    """
    获取一个log handler
